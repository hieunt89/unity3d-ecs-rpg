...
public class GUIAADropDown : MonoBehaviour {
	Dropdown mDropDown;
	...
	void OnEnable () {
		...
		if (mDropDown) {
			...
			mDropDown.onValueChanged.AddListener (delegate {
				Messenger.Broadcast <int> (Events.GUI.OnAADropDownChange, mDropDown.value);	
			});
		}
	}
	...
	void OnLoadSetting (GameSettings _gameSettings) {
		mDropDown.value = _gameSettings.antiasliasing;
	}
}
